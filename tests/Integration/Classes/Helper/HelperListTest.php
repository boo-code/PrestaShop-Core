...
class HelperListTest extends TestCase
{
    ...
    private function helper(array $skipActions): HelperList
    {
        $helper = new class() extends HelperList {
            public function viewLink($token, $id)
            {
                return $this->getViewLink($token, $id);
            }

            public function editLink($token, $id)
            {
                return $this->getEditLink($token, $id);
            }

            public function actionSkipped($action, $id)
            {
